...
/*
    ...
    - 대표적인 슬라이스 테스트 어노테이션
        1) @WebMvcTest - Controller 를 테스트 할때 주로 사용
        @WebMvcTest 선언하면 Web과 관련 Bean 만 주입이 되고, MockMvc 를 알아볼 수 있게 됨

        * MockMvc는 웹 어플리케이션을 서버에 배포하지 않고 테스트용 MVC 환경을 만들어서 요청 및 전송
        * 쉽게 말하면 내가 컨트롤러 테스트를 하고 싶을대때 실제 서버에 구현된 어플리케이션을 올리지 않고 테스트용 시뮬레이션을 돌려서 MVC 가 되도록 도와주는 클래스
 ...
 */
@WebMvcTest // 슬라이스 테스트, 컨트롤러 관련된 Bean만 로드 함
public class Ex04_1_DataRestRepository {
    /* MockMvc 테스트 방법
        MockMvc 테스트 방법
        1) MockMvc 생성 (빈 준비)
        ...
        Hal 에서 http://localhost:8080/api/article 를 돌려보면 동작하는데 왜 여기서는 안되냐면 @WebMvcTest 는 슬라이스 테스트 이기 때문에 controller 이외의 빈은 아예 로그하지 않기 때문임
        그래서 이런 경우에는 @WebMvcTest 대신 통합테스트를 사용할거임.
    */

    private final MockMvc mockMvc;

    public Ex04_1_DataRestRepository(@Autowired MockMvc mockMvc){
    ...
    void test1() throws Exception{
        ...
        /* get() 작성하는법
            ...
            5) status() 같은 방법으로 MockMvcresultMatchers.status; 선택한다(alt + enter)
