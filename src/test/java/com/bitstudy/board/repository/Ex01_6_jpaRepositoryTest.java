...
//    @Autowired Ex01_5_ArticleCommentRepository articleCommentRepository;
    ...
    @DisplayName("select 테스트") //테스트 이름달기
    @Test
    void selectTest(){
        /* given_when_then 패턴 만들고 테스트 하기 (단축코드 만들기)
            세팅(Ctrl + Alt + S) 에서 live templetes 검색
            + 버튼 > Templete group 으로 그룹 생성하고(이름은 마음대로)
            + 버튼 > Live Templete 로 단축키 설정하기
            하단에 Abbreviation 이름 넣기 (ex. gw, gwt)
            templete text 영역에 자동완성될 문구 넣기
            맨 아래 'Application in Java~' 부분의 define 또는 change 눌러서 Java 영역에서 comment, consumer function, Expression, Statement 체크하기
            그럼 앞으ㅗㄹ gw 나 gwt 라고 치면 자동완성 됨
    ...
    void updateTest(){
        ...
        */
        ...
        /* 순서 - 1) 기존의 영속성 컨텍스트 한개 엔티티(객체) 가져오기
            1. 기존의 영속성 컨텍스트로부터 하나 엔티티를 가져올건데
        ...
        */
        ...
        article.setTitle(updateHashtag); // 엔티티에 있는 setter 를 이용해서 변수를 업데이트 하기
    ...
    void deleteTest(){
        /*  기존에 데이터 있어야 되고
            ...
            1) 기존의 영속성 컨텍스트로부터 엔티티 꺼내오기
            2) 지우면 DB개수 하나 줄어드는 거니까 미리 엔티티 개수(count) 구하기
            3) 하나 삭제
            4) 2번에서 구한 개수 - 1한거랑 새로 현재 구한 count랑 비교해서 같으면 통과
