...
public class Ex01_1_Article_엔티티로_등록 {
    ...
    @Id /* 기본 키(PK) 와 객체의 필드를 매핑시켜주는 어노테이션
            ...
            기본키를 직접 할당하지 않고 DB가 대신 생성해주는 값을 사용하려면 @GeneratedValue 사용해야함 */
    ...
    *  그런데 id나 메타데이터 같은 필드들은 다른 사람이 건드리지 않고 JPA 자동으로 세팅해주는 정보들이기 때문에 @Setter 를 클래스 단위에 걸지 않고, 별도로 필요한 필드들에만 @Setter 를 달아주는걸 추천함. */
    @Setter
    ...
    // 양방향 바인딩 (Ex01_2의 // 연관관계 매핑. 다끝나고 와서 하는거)
    ...
    @ToString.Exclude /* 엄청 중요!!!
                        ...
                        @ToString이 모든 필드들을 다 찍고 Set<Ex01_2_ArticleComment>꺼를 찍으려고 하는데 그러면 ArticleComment 파일 가서도 거기있는 @ToString이 모든 원소들을 다 찍으려고 하면서 Article 이라는걸 보는 순간 다시 Article의 @ToString이 동작하면서 또 모든 원소들을 찍을거임. 이런식으로 서로가 서로를 호출하면서 순환 참조를 하게 되면서 메모리가 터져서 시스템이 다운될 수 있다.
    ...
    /* 이렇게 어노테이션만 붙여주기만 해도 auditing이 작동한다.
        ...
        리턴타입이 String 이기 때문에 createdBy 나 modified 에 들어가질 수 있다.
    ...
    */
    ...
    /* factory method pattern (정적 팩토리 메서드)
        ...
        장점: 1) static 이기 때문에 new를 이용하지 않아도 생성자를 만들 수 있다.
...
//        return Objects.equals(id, that.id);
    ...
    /*
        ...
        hashcode - 객체를 식별하는 Integer 값
                    객체의 값을 특정 알고리즘을 이용해서 계산된 정수값을 지정함
